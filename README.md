...
OpenAI is the default and production path. For a non-production proof run, the Render worker can use Gemini: set `ACCESSAGENT_AI_PROVIDER=gemini`, `GEMINI_API_KEY`, `GEMINI_VISION_MODEL=gemini-3.5-flash`, and `GEMINI_PATCH_MODEL=gemini-3.5-flash` in Render, then redeploy that worker. The Gemini adapter uses the current Interactions API, sends `store=false`, and requests native schema-constrained JSON. Both adapters receive the same screenshots, prompts, and JSON Schema, and AccessAgent validates their structured responses with Zod. Gemini proof runs use a bounded viewport and reduced accessibility-tree excerpt. For a Gemini proof, also set `ACCESSAGENT_MAX_PAGES=1` and `ACCESSAGENT_MAX_DEPTH=0` in Vercel, because Vercel queues the crawl. The OpenAI path retains full-page, high-detail evidence inspection. To return to strict OpenAI, delete that provider setting or set `ACCESSAGENT_AI_PROVIDER=openai` and redeploy. The Gemini key is inactive when the provider is OpenAI. Gemini free-tier use may be used by Google to improve its products, so use only the included demo target or another non-sensitive authorized site.
