...
export default function Home() {
  ...
  return <main className="dashboard" aria-busy={running}>
    <header className="topbar"><a className="brand" href="#audit-control" aria-label="AccessAgent home"><span className="brand-mark" aria-hidden="true">A/</span><span>AccessAgent</span></a><div className="topbar-meta">{!accessToken ? <a className="github-button" href="/auth/login">Sign in with GitHub</a> : null}<span className={`system-chip ${running ? "running" : ""}`}><i aria-hidden="true" /> {state}</span><span className="mono-chip">AUDIT + REVIEW</span></div></header>
    ...
    <section className="verification-note"><div><p className="eyebrow">VERIFIED MEANS</p><h2>Not “a patch was written.”<br />A fresh render supports the fix.</h2></div><p>When evidence is inconclusive, AccessAgent should mark the issue for human review instead of turning uncertainty into a success claim.</p></section>
  </main>;
}
