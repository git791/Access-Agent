...
export async function proposeAndApplyPatch(issues: Issue[], attempt = 1): Promise<PatchHandoff[]> {
  ...
  try {
    ...
    const fallback = `const fs=require("fs"),path=require("path");const root="/vercel/sandbox/repo",issues=JSON.parse(fs.readFileSync("/tmp/accessagent-issues.json","utf8")).issues;const description=issues.map(issue=>String(issue.id)+" "+String(issue.title)+" "+String(issue.wcag)).join(" ").toLowerCase(),needsAlt=/image-alt|alternative text/.test(description),needsLabel=/label|form elements/.test(description),needsContrast=/contrast/.test(description),files=[];const visit=dir=>{if(!fs.existsSync(dir))return;for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const item=path.join(dir,entry.name);if(entry.isDirectory())visit(item);else if(/\\.(tsx|ts|jsx|js|css)$/.test(entry.name))files.push(item);}};["app","src/app","pages","src/pages"].forEach(dir=>visit(path.join(root,dir)));let changes=0;for(const file of files){let source=fs.readFileSync(file,"utf8"),updated=source;if(needsAlt)updated=updated.replace(/<img\\b[^>]*>/g,tag=>tag.includes("alt=")?tag:tag.replace(/\\/?>(?=$)/,suffix=>' alt=""'+suffix));if(needsLabel)updated=updated.replace(/<(input|textarea)\\b[^>]*>/g,tag=>{if(tag.includes("aria-label")||tag.includes(" id="))return tag;const placeholder=tag.match(/placeholder=(['"])(.*?)\\1/);const label=placeholder?placeholder[2]:"Form field";return tag.replace(/\\/?>(?=$)/,suffix=>' aria-label="'+label.replace(/"/g,"&quot;")+'"'+suffix);});if(needsContrast)updated=updated.replace(/color:\\s*(['"])#999\\1/g,"color: '#595959'");if(updated!==source){fs.writeFileSync(file,updated);changes++;}}if(!changes)throw new Error("No deterministic accessibility fix matched the audited findings");`;
